...
- **Component APIs are identical across platforms**, using shadcn's vocabulary (`variant="ghost"`, `size="sm"`), so `pnpm ui-add` output drops into `packages/ui-web` unmodified. Mobile types its props directly from `@ken/tokens/contracts`, while `packages/ui-web/src/parity.ts` asserts the public prop unions for `Button`, `DropdownMenuItem`, and `Icon`.
- **Shared vocabulary does not mean shared pixels.** Mobile control dimensions live in `packages/ui-mobile/src/metrics.ts`; every tappable clears 44pt, and Input aligns with the default Button at 48pt. Web keeps its denser control scale.
...
- **Not shared:** screens and layout, typography (`Text` is mobile-only), `Touchable` (mobile press feedback), and Field (web-only).
- Mirrored components: `Button`, `Card`, `DropdownMenu`, `Icon`, `Input`, `Label`, `Separator`, `Skeleton`, `Theme`, and `Toast`.
- **Icons are Phosphor, one set for both platforms** — `@phosphor-icons/react` on web, `phosphor-react-native` on mobile, with identical icon names and an identical `weight` axis (`regular`, `fill`, …), so `<Icon as={HouseIcon} weight="fill" />` means the same thing everywhere. Pass the glyph in, don't wrap it: `Icon` owns only size and weight. Each app depends on its own Phosphor package directly, because call sites import the glyph themselves — `@ken/ui-web`/`@ken/ui-mobile` owning it is not enough to resolve it from an app. Three things to know:
  ...
  - Both packages ship ~1500 icons from one barrel. Web is handled: `next.config.js` sets `optimizePackageImports` (Phosphor is not in Next's default list), and `@phosphor-icons/react/House` also works as a direct subpath. **Mobile value imports must use `phosphor-react-native/src/icons/House`, never the barrel.** Metro doesn't tree-shake: one measured iOS export fell from 8.6 MiB of Phosphor Hermes bytecode to 0.19 MiB when six glyphs were deep-imported. Type-only barrel imports are erased and remain fine; the app and UI package tsconfigs redirect the source subpaths to the package declarations for typechecking.
