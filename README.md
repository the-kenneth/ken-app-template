...
- `StoreUser` (both apps) creates a `users` row the moment a session exists — instant, reload-free, race-free. It is insert-only, so running on every mount can't clobber the stored profile with stale JWT claims.
- Ownership is split: `name` belongs to the `users` table (seeded from Clerk on first sign-in, edited in-app thereafter); `email` and `imageUrl` are read-only mirrors Clerk keeps fresh. See the comments in `schema.ts`.
- The Clerk webhook (`packages/backend/convex/http.ts`) patches those mirrors and cascades deletions.
